...
export default function App() {
  const [messages, setMessages] = useState([
    { role: 'assistant', content: 'Hello! I’m your AI assistant with web knowledge, file reading, and voice input. Ask me anything!' }
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: '❌ Failed to connect. Is the backend running?' }
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const msg = `📄 I've read your file: "${file.name}". Here's a summary:\n\n"${data.text.substring(0, 500)}..."\n\nAsk me questions about it!`;
      setMessages((prev) => [...prev, { role: 'assistant', content: msg }]);
    } catch (err) {
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: '❌ Failed to read file. Try PDF, Word, or TXT.' }
      ]);
    }
  };
